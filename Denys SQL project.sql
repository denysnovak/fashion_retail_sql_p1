...
create table cleaned_fashion_retail_sales as
select distinct *
from fashion_retail_sales
where `Customer Reference ID` is not null and trim(`Customer Reference ID`) <> ''
and `Item Purchased` is not null and trim(`Item Purchased`) <> ''
and `Purchase Amount (USD)` is not null and trim(`Purchase Amount (USD)`) <> ''
and `Date Purchase` is not null and trim(`Date Purchase`) <> ''
and `Review Rating` is not null and trim(`Review Rating`) <> ''
and `Payment Method` is not null and trim(`Payment Method`) <> '';
